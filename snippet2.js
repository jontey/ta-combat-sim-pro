var test = {
	...
	initialize : function() {
		this.doneSound = new Audio(
				"https://dl.dropbox.com/u/41023713/tasim_done_snd.wav");
		this.add_ViewModeChange = (new ClientLib.Vis.ViewModeChange).$ctor(
		...
		setTimeout(
				function() {
					try {
						...
						var player = ClientLib.Data.MainData.GetInstance$9()
								.get_Player$2();
						_this.attacker_modules.l = [];
						for ( var i in g.m_Gamedata.units) {
							var ug = g.GetUnit$0(i);
							var research = player.m_PlayerResearch
		...
		this.troopDamageLabel = new qx.ui.basic.Label()
				.set({
		...
		this.add_ArmyChanged = (new System.EventHandler).$ctor(this,
				this.onUnitMoved);
		...
		this.ajaxImage = new qx.ui.basic.Image(
				"https://dl.dropbox.com/u/41023713/loading.gif");
		...
		var primarySelectDefault = new qx.ui.form.ListItem("C. Yard", null,
				"CY");
		this.primarySelect.add(primarySelectDefault);
		this.primarySelect.add(new qx.ui.form.ListItem("Repair Time", null,
				"RT"));
		this.primarySelect.add(new qx.ui.form.ListItem("Troop Strength", null,
				"TS"));
		this.primarySelect.add(new qx.ui.form.ListItem("Defense Facility",
				null, "DF"));
		this.primarySelect.add(new qx.ui.form.ListItem("Enemy Strength", null,
				"ES"));
		this.primarySelect.add(new qx.ui.form.ListItem("Battle Time", null,
				"BT"));
		...
		this.secondarySelect
				.add(new qx.ui.form.ListItem("C. Yard", null, "CY"));
		var secondarySelectDefault = new qx.ui.form.ListItem("Repair Time",
				null, "RT");
		this.secondarySelect.add(secondarySelectDefault);
		this.secondarySelect.add(new qx.ui.form.ListItem("Troop Strength",
				null, "TS"));
		this.secondarySelect.add(new qx.ui.form.ListItem("Defense Facility",
				null, "DF"));
		this.secondarySelect.add(new qx.ui.form.ListItem("Enemy Strength",
				null, "ES"));
		this.secondarySelect.add(new qx.ui.form.ListItem("Battle Time", null,
				"BT"));
		...
		this.tertiarySelect.add(new qx.ui.form.ListItem("Repair Time", null,
				"RT"));
		this.tertiarySelect.add(new qx.ui.form.ListItem("Troop Strength", null,
				"TS"));
		this.tertiarySelect.add(new qx.ui.form.ListItem("Defense Facility",
				null, "DF"));
		var tertiarySelectDefault = new qx.ui.form.ListItem("Enemy Strength",
				null, "ES");
		this.tertiarySelect.add(tertiarySelectDefault);
		this.tertiarySelect.add(new qx.ui.form.ListItem("Battle Time", null,
				"BT"));
	...
	updateLayoutsList : function() {
		...
		if (layouts) {
			for ( var i in layouts) {
				...
				var item = new qx.ui.form.ListItem(layout.label, null,
						layout.id);
	...
	deleteCityLayout : function() {
		try {
			...
			if (layouts && typeof layouts[current_city] != 'undefined'
					&& typeof layouts[current_city][lid] != 'undefined') {
	...
	calculateLoot : function() {
		...
		for ( var j = num; --j >= 0;) {
			...
			for ( var i = building.m_UnitLevelRequirements.rer.length; --i >= 0;) {
				spoils[building.m_UnitLevelRequirements.rer[i].t] += mod
						* building.m_UnitLevelRequirements.rer[i].c;
			}
		}
		...
		if (city.m_CityUnits.m_DefenseUnits != null) {
			...
			for (j = num; --j >= 0;) {
				...
				for (i = unit.m_UnitLevelRequirements.rer.length; --i >= 0;) {
					spoils[unit.m_UnitLevelRequirements.rer[i].t] += mod
							* unit.m_UnitLevelRequirements.rer[i].c;
				}
			}
		}
	...
	optimizeLayout : function() {
		try {
			...
			if (this.optimizing) {
				...
				this.updateProWindow();
				this.setTargets();
				this.degree = parseInt(this.degreeSelect.getSelection()[0]
						.getModel());
	...
	calculateTroopStrengths : function(battleground) {
		...
		for ( var i in entities) {
			...
			if (i_entity.m_eAlignment == attacker) {
				...
				if (entity.m_Type == 1) {
					...
					if (i_entity.m_MDCTypeId >= 200
							&& i_entity.m_MDCTypeId <= 205) {
						/* 200, FALCON SUPPORT
		...
		this.lastInfantryRepairTime = crd.ConvertRepairCost$0(
				r_types.RepairChargeInf,
				repair_times[ClientLib.Data.EUnitGroup.Infantry],
				(1 - this.lastInfantryPercentage / 100));
		this.lastAircraftRepairTime = crd.ConvertRepairCost$0(
				r_types.RepairChargeAir,
				repair_times[ClientLib.Data.EUnitGroup.Aircraft],
				(1 - this.lastAirPercentage / 100));
		this.lastVehicleRepairTime = crd.ConvertRepairCost$0(
				r_types.RepairChargeVeh,
				repair_times[ClientLib.Data.EUnitGroup.Vehicle],
				(1 - this.lastVehiclePercentage / 100));
	...
	onDamageDone : function(sender, e) {
		...
		try {
			...
			if (battleground.m_CurrentStep % 10 == 0) {
				window.TASuite.main.getInstance().updateTroopStrength(
						battleground);
			}
		} catch (e) {
			console.log(e);
	...
	updateProWindow : function() {
		...
		this.updateLabel100(this.enemyTroopStrengthLabel,
				this.lastEnemyPercentage, -1); // current names
		// -DEFENCES
	...
	updateTroopStrength : function(battleground) {
		...
		this.troopDamageLabel
				.setValue("<span style='color: black; font-weight: bold;'>Troop Strength: "
	...
	bustCache : function() {
		...
		try {
			var own_city = ClientLib.Data.MainData.GetInstance().get_Cities()
					.get_CurrentOwnCity();
			own_city.m_CityArmyFormationsManager.m_ArmyFormations.d[own_city.m_CityArmyFormationsManager.m_CurrentTargetBaseId]
					.CopyCityOffenseUnitsLayout$0();
		} catch (e) {
			console.log(e);
		}
	},
	setupBattleground : function(offense_units) {
		try {
			...
			offense_units = offense_units
					|| own_city.m_CityArmyFormationsManager.m_ArmyFormations.d[current_city
							.get_Id()];
			...
			for ( var i = 0; (i < 9); i++) {
				for ( var j = 0; (j < 8); j++) {
					var terrainType = current_city.GetResourceType$0(i,
							(j + current_city.get_CityHeight$0()));
					var unitType = -1;
					switch (terrainType) {
			...
			combatData.m_AttackerAllianceName = own_city
			...
			battleground.m_BattleDuration = (1200 * Math
					.floor(0x3e8 / battleground.m_SimSetup.get_SubSteps$0()));
	...
	startSimulation : function() {
		try {
			...
			window.TASuite.main.getInstance().troopDamageLabel
					.setValue("<span style='color: black; font-weight: bold;'>Troop Strength: 100%</span>");
			...
			battleground.m_Simulation
					.add_DamageDone$0((new System.EventHandler).$ctor(this,
							this.onDamageDone));
			battleground.m_Simulation
					.add_OnDestroyDefense$0((new System.EventHandler).$ctor(
							this, this.onDefenseDestroyed));
